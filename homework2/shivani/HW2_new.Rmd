...
#OBSERVATIONS
#1. Vector operations are several times faster than for loops, ranging from 2x-10x
#2. As size of vector increases, for loop performance degrades significantly, but vectorised operations performance does not degrade that fast  
...
```{r}
#Group by provider_state and county, because states have many same county names, eg Washington county is present in 31 counties
covid_df %>%
...
final_shortage_df %>%
  mutate(nursing_shortage_ratio = total_shortage_facilities/total_facilities) %>%
  filter(nursing_shortage_ratio > 0.3)
#---------------------CLINICAL STAFF----------------------
#-----------------------------------------
#Calculate number of weeks of shortage for each facility per state and filter facilities that have more than 20 weeks of shortage
shortage_weeks_df <- covid_df %>% #Subset data to increase efficiency
...
#Merge total facilities and facilities with 20 weeks shortage by state. Filter states with a 0.05 ratio of shortage facilities: total facilities
...
```{r}
#Total number of facilities in each state and county, to calculate ratio of shortage facilities
#Group by state and county, because multiple states have same county names eg AL Washington, AR Washington etc
county_facilities <- covid_df %>% group_by(provider_state, county) %>% 
  summarise(total_facilities = n_distinct(federal_provider_number))
county_facilities

#-------------------N95 MASKS------------------------
#-----------------------------------------
#Calculate number of weeks of shortage for each facility per county and filter facilities that have more than 10 weeks of shortage
shortage_weeks_df <- covid_df %>% #Subset data to increase efficiency
...
final_shortage_df <- merge( county_shortage, county_facilities, by = c("county", "provider_state"))
final_shortage_df %>%
...
#Calculate number of weeks of shortage for each facility per state and filter facilities that have more than 20 weeks of shortage
shortage_weeks_df <- covid_df %>% #Subset data to increase efficiency
  select(provider_state, county, federal_provider_number, week_ending, one_week_supply_of_surgical_masks) %>% 
...
final_shortage_df %>%
  ...
  filter(surgical_mask_shortage_ratio > 0.8)

#-------------------EYE PROTECTION------------------------
#-----------------------------------------
#Calculate number of weeks of shortage for each facility per state and filter facilities that have more than 20 weeks of shortage
shortage_weeks_df <- covid_df %>% #Subset data to increase efficiency
  select(provider_state, county, federal_provider_number, week_ending, one_week_supply_of_eye_protection) %>% 
...
final_shortage_df %>%
  ...
  filter(eye_protection_shortage_ratio > 0.8)

#-------------------GOWNS------------------------
#-----------------------------------------
#Calculate number of weeks of shortage for each facility per state and filter facilities that have more than 20 weeks of shortage
shortage_weeks_df <- covid_df %>% #Subset data to increase efficiency
  select(provider_state, county, federal_provider_number, week_ending, one_week_supply_of_gowns) %>% 
...
final_shortage_df %>%
  ...
  filter(gowns_shortage_ratio > 0.8)
...
#Calculate number of weeks of shortage for each facility per state and filter facilities that have more than 20 weeks of shortage
shortage_weeks_df <- covid_df %>% #Subset data to increase efficiency
  select(provider_state, county, federal_provider_number, week_ending, one_week_supply_of_gloves) %>% 
...
final_shortage_df %>%
  ...
  filter(gloves_shortage_ratio > 0.8)

#-------------------HAND SANITIZER------------------------
#-----------------------------------------
#Calculate number of weeks of shortage for each facility per state and filter facilities that have more than 20 weeks of shortage
shortage_weeks_df <- covid_df %>% #Subset data to increase efficiency
  select(provider_state, county, federal_provider_number, week_ending, one_week_supply_of_hand_sanitizer) %>% 
...
final_shortage_df %>%
  ...
  filter(hand_sanitizer_shortage_ratio > 0.8)
...
covid_df %>%
  ...
  summarise(total_weeks_of_shortage = n_distinct(week_ending)) %>%
  filter(total_weeks_of_shortage > 10)
```